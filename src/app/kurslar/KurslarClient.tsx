...
export default function KurslarClient({ initialProducts, categories }: { initialProducts: any[], categories: any[] }) {
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (selectedCategory !== "Tümü") {
      result = result.filter(c => c.categories?.some((cat: any) => cat.id === selectedCategory));
    }
                    ...
                    {course.categories && course.categories.length > 0 && (
                      <div className="absolute top-4 left-4 flex flex-col gap-2 items-start z-10">
                        {course.categories.map((cat: any) => (
                          <div key={cat.id} className="bg-white px-3 py-1.5 rounded-lg text-xs font-bold text-slate-900 shadow-sm">
                            {cat.name}
                          </div>
                        ))}
                      </div>
