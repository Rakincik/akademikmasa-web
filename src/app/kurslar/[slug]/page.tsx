      ...
      <div className="bg-slate-900 text-white pt-12 pb-40 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-400 mb-8 font-bold tracking-wide">
            ...
            <span className="opacity-50">/</span>
            <span className="text-brand-500">{course.categories?.length > 0 ? course.categories.map((c: any) => c.name).join(", ") : "KATEGORİSİZ"}</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative z-20">
              ...
              <Link href="/kadromuz" className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-10 bg-white/5 border border-white/10 hover:border-brand-500/50 hover:bg-white/10 transition-all duration-300 rounded-3xl p-4 sm:pr-8 w-fit max-w-full backdrop-blur-xl shadow-xl group cursor-pointer">
                <div className="flex flex-wrap -space-x-3 sm:-space-x-4 shrink-0">
                  {course.instructors.slice(0, 5).map((inst, idx) => (
                    <img 
                      key={idx}
                      src={inst.imageUrl || "https://ui-avatars.com/api/?name=" + encodeURIComponent(inst.name)} 
                      alt={inst.name} 
                      className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-slate-900 shadow-[0_0_15px_rgba(191,29,48,0.5)] group-hover:border-brand-500 group-hover:scale-110 transition-all duration-300 object-cover relative" 
                      style={{ zIndex: course.instructors.length - idx }}
                    />
                  ))}
                  {course.instructors.length > 5 && (
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-slate-900 shadow-[0_0_15px_rgba(191,29,48,0.5)] bg-slate-800 flex items-center justify-center text-xs font-bold text-white relative z-0">
                      +{course.instructors.length - 5}
                    </div>
                  )}
                </div>
                <div className="max-w-full">
                  <p className="text-[0.65rem] text-brand-300 font-black uppercase tracking-widest mb-1">
                    {course.instructors.length > 1 ? "KURS EĞİTMENLERİ" : "KURS EĞİTMENİ"}
                  </p>
                  <p className="text-sm sm:text-base text-white font-bold leading-snug break-words">
      ...
      <div className="container mx-auto px-4 -mt-20 relative z-30">
        <div className="flex flex-col lg:flex-row gap-10 items-start">
          ...
          <div className="w-full lg:w-2/3 bg-white/80 backdrop-blur-2xl rounded-[3rem] p-10 md:p-14 shadow-2xl shadow-slate-200/50 border border-white">
            ...
            <div 
              className="prose prose-slate prose-lg max-w-none w-full overflow-hidden text-slate-600 mb-16 leading-relaxed font-medium"
              dangerouslySetInnerHTML={{ __html: (course.longDescription || "").replace(/&nbsp;/g, " ") }}
            />
