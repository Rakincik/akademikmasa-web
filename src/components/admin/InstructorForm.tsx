...
export default function InstructorForm({ action }: { action: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataForAction.append("branch", branch);
      formDataForAction.append("department", department);
      formDataForAction.append("motto", motto);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          ...
          <input 
            ...
            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all"
            placeholder="Örn: 4 Temel Beceri"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-2">Departman / Kategori</label>
          <input 
            type="text" 
            value={department}
            onChange={(e) => setDepartment(e.target.value)}
            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all"
            placeholder="Örn: Türkçe ÖABT"
          />
        </div>
      </div>

      <div className="mb-6">
        <label className="block text-sm font-medium text-slate-700 mb-2">Eğitmen Mottosu (Sözü)</label>
        <textarea 
          value={motto}
          ...
          placeholder="Örn: Eğitimde sınırları zorlayan, yenilikçi yaklaşımlar."
        ></textarea>
      </div>
